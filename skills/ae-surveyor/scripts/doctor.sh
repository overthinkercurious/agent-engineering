...
if [ ! -f "$GI" ]; then
  ...
  if ae_self_hosted "$ROOT"; then
    ae_info "self-hosted: skills/ is the product here, so no suite ignore rule is expected"
  else
    miss=""
...
if (cd "$ROOT" && git rev-parse --git-dir >/dev/null 2>&1) && ! ae_self_hosted "$ROOT"; then
...
if ae_self_hosted "$ROOT"; then
  ae_info "self-hosted: no skills-lock.json is expected in the kit's own repository"
elif [ -f "$ROOT/skills-lock.json" ]; then
...
# ---------------------------------------------------- generator version ----
# A survey could not previously say it predated a fix: the stamp recorded what
# was read (snapshot, input sha, commit) and never what read it, so an artifact
# written by an older kit looked exactly like a current one.

ae_head "generator"
KITJSON="$AE_SELF/../references/kit.json"
if [ ! -f "$KITJSON" ]; then
  ae_info "no kit.json beside the scripts; generator version unavailable"
else
  installed="$(tr -d ' \t\r' < "$KITJSON" | sed -n 's/.*"version":"\([^"]*\)".*/\1/p' | head -1)"
  stamped="$(cat "$ROOT"/.dev/knowledge/*.md 2>/dev/null | sed -n 's/.*agent-engineering:kit:\([^ ]*\) -->.*/\1/p' | sort -u | head -2 | tr '\n' ' ' | sed 's/ *$//')"
  if [ -z "$stamped" ]; then
    ae_info "no generated block carries a kit version yet - regenerate to stamp one"
  elif [ "$stamped" = "$installed" ]; then
    ae_ok "knowledge base was generated by the installed kit ($installed)"
  else
    warn "knowledge base was generated by kit [$stamped] but [$installed] is installed - re-run the survey before trusting its tags"
  fi
fi
